...
export default function ExperienceDetails({ position, company, time, address, work }: ExperienceDetailsProp): JSX.Element {
  ...
  return (
    <li ref={ref} className='my-8 first:mt-0 last:mb-0 w-[60%] mx-auto flex flex-col items-start justify-between mini-tablet:w-[80%]'>
      ...
      >
        <h3 className='capitalize font-bold text-2xl pb-2 mobile:text-xl mini-mobile:text-lg'>{position}&nbsp;<span className='text-primary capitalize'>@{company}</span></h3>
        <span className='capitalize font-medium text-dark/75 dark:text-light/75 mini-mobile:text-sm'>
          {time} | {address}
        </span>
        <ul className='font-medium w-full list-disc pt-2 mini-mobile:text-sm'>
          {
